...
// AmbiguousCommitError represents an error that has a transaction into an
...
// AmbiguousCommitError implements error.
var _ error = &AmbiguousCommitError{}
...
func ExecuteTx(db *sql.DB, fn func(*sql.Tx) error) (err error) {
	...
	for {
		...
		if retryable := ok && pqErr.Code == "CR000"; !retryable {
			if released {
				err = &AmbiguousCommitError{inner: err}
			}
			return err
		}
