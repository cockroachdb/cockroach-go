...
func downloadLatestBinary(tc *TestConfig, nonStable bool) (string, error) {
...
func GetDownloadFilename(
	...
	return fmt.Sprintf("cockroach-%s", latestStableVersion), nil
}
...
func downloadBinaryFromTar(response *http.Response, output *os.File, filePath string) error {
	// Unzip the tar/zip file from the response's body.
	...
	for {
		...
		if err == io.EOF {
			return fmt.Errorf("cannot find the binary")
		}
